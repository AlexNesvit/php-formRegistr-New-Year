...
if (isset($_POST['validate'])) {
    ...
    if (empty($errors)) {
        ...
        $req = $pdo->prepare("INSERT INTO don  SET `Title`=?, `user_iduser` = ?, `categorie_idcategorie` = ?");
        $title = htmlspecialchars($_POST['title']);
        $category = htmlspecialchars($_POST['category']);
        $user_id = $_SESSION['auth']->id;
        $req->execute([$title, $user_id, $category]);
        $_SESSION['flash']['success'] = 'Votre don a bien été créé';
